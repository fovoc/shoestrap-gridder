...
define( 'SHOESTRAP_GRIDDER_STORE_URL', 'http://bootstrap-commerce.com/downloads' );
define( 'SHOESTRAP_GRIDDER_ITEM_NAME', 'Shoestrap MP Addon' );
...
// setup the updater
$edd_updater = new EDD_SL_Plugin_Updater( SHOESTRAP_GRIDDER_STORE_URL, __FILE__, array( 
    'version'   => '1.00',         // current version number
    'license'   => $license_key,
    'item_name' => SHOESTRAP_GRIDDER_ITEM_NAME,
  ...
  <div class="postbox">
    ...
    <div class="inside">
      <form method="post" action="options.php">
        ...
        <label class="description" for="shoestrap_gridder_license_key">
...
function shoestrap_gridder_register_option() {
  ...
  register_setting('shoestrap_gridder_license', 'shoestrap_gridder_license_key', 'shoestrap_gridder_sanitize_license' );
}

function shoestrap_gridder_sanitize_license( $new ) {
...
function shoestrap_gridder_activate_license() {
  ...
  if( get_option( 'shoestrap_gridder_license_status' ) == 'active' )
    return;
  ...
  $api_params = array( 
    ...
    'license'   => $license, 
    'item_name' => urlencode( SHOESTRAP_GRIDDER_ITEM_NAME ) 
  );
  
  // Call the custom API.
  $response = wp_remote_get( add_query_arg( $api_params, SHOESTRAP_GRIDDER_STORE_URL ) );
