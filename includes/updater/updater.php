...
if ( ! function_exists( 'shoestrap_gridder_license_form' ) ) {
	function shoestrap_gridder_license_form() {
		...
		<tr id="shoestrap-gridder" class="<?php echo $status; ?>">
			<form method="post" action="options.php">
				<?php settings_fields( 'shoestrap_gridder_license' ); ?>
				<td class="plugin-title">
					<strong><?php _e( 'Shoestrap 3 Gridder Addon', 'shoestrap_gridder' ); ?></strong>
					<div class="row-actions visible">
						<?php wp_nonce_field( 'shoestrap_gridder_nonce', 'shoestrap_gridder_nonce' ); ?>

						<?php if ( false !== $license ) : ?>
							<?php if ( $status !== false && $status == 'valid' ) : ?>
								<input type="submit" class="button-deactivate" name="shoestrap_gridder_license_deactivate" value="<?php _e( 'Deactivate License', 'shoestrap_gridder' ); ?>"/>
							<?php else : ?>
								<input type="submit" class="button-activate" name="shoestrap_gridder_license_activate" value="<?php _e( 'Activate License', 'shoestrap_gridder' ); ?>"/>
							<?php endif; ?>
						<?php endif; ?>
					</div>
				</td>
